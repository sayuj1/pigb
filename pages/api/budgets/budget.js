...
export default async function handler(req, res) {
  ...
  switch (req.method) {
    ...
    case "POST":
      try {
        const { category, startDate, endDate, limitAmount } = req.body;

        if (!category || !startDate || !endDate || !limitAmount) {
          return res.status(400).json({ message: "Missing required fields" });
        }

        const newBudget = await Budget.createBudget(userId, category, new Date(startDate), new Date(endDate), limitAmount);
    ...
    case "GET":
      try {
        ...
        // Build search filter
        const searchFilter = searchQuery ? { category: { $regex: searchQuery, $options: "i" } } : {};
        ...
        const totalBudgets = await Budget.countDocuments({ userId, ...searchFilter });
    ...
    case "PUT":
        try {
            ...
            const { _id: id, category, limitAmount, startDate, endDate } = req.body;
            ...
            if (budget.userId.toString() !== userId) {
              return res.status(403).json({ message: "You are not authorized to edit this budget" });
            }
            ...
            const startDateChanged = new Date(startDate).getTime() !== new Date(oldStartDate).getTime();
            const endDateChanged = new Date(endDate).getTime() !== new Date(oldEndDate).getTime();
            ...
            if (startDateChanged || endDateChanged || categoryChanged) {
              ...
              for (const transaction of budget.transactions) {
                const transactionDetails = await Transaction.findById(transaction.transactionId);
                
                // If transaction falls within the old budget's date range or category, remove it from the budget
                if (
                  (transactionDetails.date >= oldStartDate && transactionDetails.date <= oldEndDate) &&
            ...
            if (startDateChanged || endDateChanged || categoryChanged) {
              ...
              for (const transaction of transactionsToAdd) {
                await Budget.addExpense(budget.userId, category, transaction.date, transaction.amount, transaction._id, transaction.description);
              }
            }
