...
export default async function handler(req, res) {
    ...
    switch (req.method) {
        ...
        case "POST":
            const { loanType, borrowerName, lenderName, loanCategory, amount, interestRate, tenureMonths, dueDate } = req.body;

            if (!loanType || !["taken", "given"].includes(loanType)) {
                return res.status(400).json({ error: "Invalid loan type" });
            }

            if (!loanCategory || !["personal", "business", "home", "car", "education", "other"].includes(loanCategory)) {
                return res.status(400).json({ error: "Invalid loan category" });
            }

            if (loanType === "taken" && !borrowerName) {
                return res.status(400).json({ error: "Borrower name is required for taken loans" });
            }

            if (loanType === "given" && !lenderName) {
                return res.status(400).json({ error: "Lender name is required for given loans" });
            }
        ...
        case "GET":
            try {
                ...
                // Searching by borrowerName or lenderName
                if (search) {
                  query.$or = [
        ...
        case "PUT":
            try {
                ...
                // `pre("save")` middleware will handle EMI, remaining balance, and status update
                await loan.save();
